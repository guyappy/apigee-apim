...
const { deployProxy, undeployProxy, deployExistingRevision, listDeployedRevision, downloadProxies, deployProxies, downloadProxy } = require('./proxy')
...
program.command('downloadProxy')
  .requiredOption('-n, --api <name>', 'The name of the API proxy.')
  .action((options) => downloadProxy(build(), options.api).catch(handleError))
